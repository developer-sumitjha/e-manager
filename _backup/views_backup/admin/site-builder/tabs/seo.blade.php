    ...
    <form id="seoForm">
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="meta_title" value="{{ $settings->meta_title }}" maxlength="60" placeholder="Your Store Name - Best Products Online">
                <small class="text-muted">Maximum 60 characters</small>
            </div>
            
            <div class="form-group">
                <label class="form-label">Meta Description</label>
                <textarea class="form-control" name="meta_description" rows="3" maxlength="160" placeholder="Describe your store in 160 characters...">{{ $settings->meta_description }}</textarea>
                <small class="text-muted">Maximum 160 characters</small>
            </div>
            
            <div class="form-group">
                <label class="form-label">Meta Keywords</label>
                <input type="text" class="form-control" name="meta_keywords" value="{{ $settings->meta_keywords }}" placeholder="ecommerce, online shopping, products">
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="google_analytics_code" rows="3" placeholder="<!-- Google Analytics -->">{{ $settings->google_analytics_code }}</textarea>
            </div>
            
            <div class="form-group">
                <label class="form-label">Facebook Pixel Code</label>
                <textarea class="form-control" name="facebook_pixel_code" rows="3" placeholder="<!-- Facebook Pixel -->">{{ $settings->facebook_pixel_code }}</textarea>
            </div>
        </div>
...
function saveSeo() {
    ...
    fetch('{{ route('admin.site-builder.update-seo') }}', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (ogFile) {
                ...
                return fetch('{{ route('admin.site-builder.upload-og-image') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Accept': 'application/json'
