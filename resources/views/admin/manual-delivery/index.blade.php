...
@section('title', 'Manual Delivery System')
@section('page-title', 'Manual Delivery System')
...
<div class="manual-delivery-header">
    ...
    <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
        <a href="{{ route('admin.manual-delivery.allocation') }}" class="btn btn-primary" style="padding: 0.75rem 1.5rem; border-radius: 0.75rem; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 0.5rem;">
        ...
        <div class="summary-badges">
            <div class="summary-badge">
                ...
                <span>{{ $metrics['active_riders'] }} Active Riders</span>
            </div>
            <div class="summary-badge">
...
<div class="metrics-grid">
    ...
    <div class="metric-card">
        ...
        <div class="metric-value">{{ $metrics['active_riders'] }}</div>
    </div>
    ...
    <div class="metric-card">
        ...
        <div class="metric-value">₹{{ number_format($metrics['revenue'], 2) }}</div>
    </div>
</div>

<div class="sub-nav-tabs">
    <a href="{{ route('admin.manual-delivery.index', ['tab' => 'overview']) }}" class="sub-nav-tab {{ $activeTab == 'overview' ? 'active' : '' }}">
        <i class="fas fa-box icon"></i>
        <span>Overview</span>
    </a>
    <a href="{{ route('admin.manual-delivery.allocation') }}" class="sub-nav-tab {{ request()->routeIs('admin.manual-delivery.allocation') ? 'active' : '' }}">
        <i class="fas fa-truck icon"></i>
        <span>Order Allocation</span>
    </a>
    <a href="{{ route('admin.manual-delivery.deliveries') }}" class="sub-nav-tab">
        <i class="fas fa-map-marker-alt icon"></i>
        <span>Manual Deliveries</span>
    </a>
    <a href="{{ route('admin.manual-delivery.activities') }}" class="sub-nav-tab">
        <i class="fas fa-chart-line icon"></i>
        <span>Activities & Reports</span>
    </a>
    <a href="{{ route('admin.manual-delivery.delivery-boys') }}" class="sub-nav-tab">
        <i class="fas fa-user icon"></i>
        <span>Delivery Boys</span>
    </a>
    <a href="{{ route('admin.manual-delivery.performance') }}" class="sub-nav-tab">
        <i class="fas fa-chart-line icon"></i>
        <span>Performance</span>
    </a>
    <a href="{{ route('admin.manual-delivery.cod-settlements') }}" class="sub-nav-tab">
...
<div class="content-sections">
    <div class="section-card">
        ...
        <div class="delivery-item">
            <div class="delivery-info">
                <div class="delivery-customer">{{ $delivery->order->user->name ?? 'N/A' }}</div>
                <div class="delivery-phone">{{ $delivery->order->user->email ?? 'N/A' }}</div>
            </div>
            <div class="delivery-status">
                <span class="status-badge {{ $delivery->status }}">{{ ucfirst($delivery->status) }}</span>
                <span class="delivery-amount">₹{{ number_format($delivery->delivery_fee, 2) }}</span>
            </div>
        </div>
        @empty
    ...
    <div class="section-card">
        ...
        @forelse($activeDeliveryBoys as $boy)
        <div class="delivery-boy-item">
            <div class="delivery-boy-info">
                <div class="delivery-boy-name">{{ $boy->name }}</div>
                <div class="delivery-boy-phone">{{ $boy->phone }}</div>
            </div>
            <div class="success-rate">{{ $boy->calculated_success_rate ?? 100 }}% Success Rate</div>
        </div>
        @empty
        <div class="text-center py-4">
