...
<div class="ledger-summary">
    ...
    <div class="summary-card">
        ...
        <div class="summary-value {{ $netBalance >= 0 ? 'balance-positive' : 'balance-negative' }}">
...
<div class="filters-bar">
    <div class="search-box">
        ...
        <input type="text" id="ledger-search" class="form-control" placeholder="Search transactions..." value="{{ request('search') }}">
    </div>
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if(!$isIncome)
                        <div class="amount-value amount-debit">₹{{ number_format($transaction->amount, 2) }}</div>
                        @else
                    ...
                    <td>
                        @if($isIncome)
                        <div class="amount-value amount-credit">₹{{ number_format($transaction->amount, 2) }}</div>
                        @else
                        <div class="amount-value">-</div>
                        @endif
                    </td>
                    <td>
                        <div class="reference-number">{{ $transaction->reference ?? 'N/A' }}</div>
                    </td>
                    <td>
                        <div class="d-flex gap-2">
                            <a href="{{ route('admin.accounting.transactions.edit', $transaction->id) }}" class="action-btn edit">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <button type="button" class="action-btn delete" onclick="deleteTransaction({{ $transaction->id }})">
            ...
            <tfoot>
                <tr style="background: rgba(16, 185, 129, 0.05); font-weight: 600;">
                    ...
                    <td colspan="4">
                        <div class="running-balance {{ $runningBalance >= 0 ? 'balance-positive' : 'balance-negative' }}">
...
<script>
    ...
    function applyFilters() {
        ...
        window.location.href = `{{ route('admin.accounting.ledger') }}?${params.toString()}`;
    }
