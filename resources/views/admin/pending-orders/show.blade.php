...
@section('page-title', 'Pending Order Details')
@section('page-subtitle', 'View and manage pending order information')
...
@section('breadcrumb')
    ...
    <div class="breadcrumb-item active">Order #{{ $pending_order->order_number }}</div>
@endsection
...
<div class="row">
    ...
    <div class="col-xl-8 mb-4">
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="card-title mb-0">Order #{{ $pending_order->order_number }}</h5>
                        <small class="text-muted">Created {{ $pending_order->created_at->format('M j, Y \a\t g:i A') }}</small>
                    </div>
                    <div class="d-flex gap-2">
                        <span class="badge badge-warning">
                            {{ ucfirst($pending_order->status) }}
                        </span>
                        <span class="badge badge-{{ $pending_order->payment_status === 'paid' ? 'success' : ($pending_order->payment_status === 'refunded' ? 'warning' : 'danger') }}">
                            {{ ucfirst($pending_order->payment_status) }}
                        </span>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @foreach($pending_order->orderItems as $item)
                            <tr>
                                <td>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td class="fw-semibold">Rs. {{ number_format($pending_order->subtotal ?? $pending_order->orderItems->sum('total'), 2) }}</td>
                            </tr>
                            @if($pending_order->tax_amount > 0)
                            <tr>
                                <td colspan="3" class="text-end">Tax:</td>
                                <td>Rs. {{ number_format($pending_order->tax_amount, 2) }}</td>
                            </tr>
                            @endif
                            @if($pending_order->shipping_cost > 0)
                            <tr>
                                <td colspan="3" class="text-end">Shipping:</td>
                                <td>Rs. {{ number_format($pending_order->shipping_cost, 2) }}</td>
                            </tr>
                            @endif
                            <tr class="table-active">
                                <td colspan="3" class="text-end fw-bold">Total:</td>
                                <td class="fw-bold">Rs. {{ number_format($pending_order->total, 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
        ...
        <div class="card mt-4">
            ...
            <div class="card-body">
                <p class="mb-0">{{ $pending_order->notes }}</p>
            </div>
        </div>
        @endif
    </div>
    ...
    <div class="col-xl-4">
            ...
            <div class="card-body">
                <div class="d-flex align-items-center mb-3">
                    <div class="avatar-lg me-3">
                        <div class="avatar-initials">
                            {{ substr($pending_order->receiver_name ?? $pending_order->user->name ?? 'G', 0, 1) }}
                        </div>
                    </div>
                    <div>
                        <div class="fw-semibold">{{ $pending_order->receiver_name ?? $pending_order->user->name ?? 'Guest' }}</div>
                        <small class="text-muted">{{ $pending_order->receiver_phone ?? $pending_order->user->phone ?? 'N/A' }}</small>
                    </div>
                </div>
                ...
                <div class="row g-2">
                    <div class="col-12">
                        ...
                        <div class="fw-semibold">{{ $pending_order->receiver_phone ?? $pending_order->user->phone ?? 'N/A' }}</div>
                    </div>
                    @if($pending_order->user)
                    <div class="col-12">
                        <small class="text-muted">Email</small>
                        <div class="fw-semibold">{{ $pending_order->user->email ?? 'N/A' }}</div>
                    </div>
                    <div class="col-12">
                        <small class="text-muted">Role</small>
                        <div class="fw-semibold">{{ ucfirst($pending_order->user->role ?? 'Guest') }}</div>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <div class="fw-semibold">{{ $pending_order->created_at->format('M j, Y H:i') }}</div>
                    </div>
                    <div class="col-12">
                        <small class="text-muted">Last Updated</small>
                        <div class="fw-semibold">{{ $pending_order->updated_at->format('M j, Y H:i') }}</div>
                    </div>
                    <div class="col-12">
                        <small class="text-muted">Payment Method</small>
                        <div class="fw-semibold">{{ ucfirst(str_replace('_', ' ', $pending_order->payment_method ?? 'N/A')) }}</div>
                    </div>
                    @if($pending_order->createdBy)
                    <div class="col-12">
                        <small class="text-muted">Created By</small>
                        <div class="fw-semibold">{{ $pending_order->createdBy->name ?? 'N/A' }}</div>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <!-- Shipping Address -->
        @if($pending_order->shipping_address || $pending_order->receiver_name)
        <div class="card mb-4">
            <div class="card-header">
                <h6 class="card-title mb-0">Shipping Address</h6>
            </div>
            <div class="card-body">
                @if($pending_order->receiver_name)
                <div class="fw-semibold mb-2">{{ $pending_order->receiver_name }}</div>
                @endif
                @if($pending_order->receiver_phone)
                <div class="text-muted mb-2">{{ $pending_order->receiver_phone }}</div>
                @endif
                @if($pending_order->receiver_full_address)
                <div class="text-muted mb-2">{{ $pending_order->receiver_full_address }}</div>
                @elseif($pending_order->shipping_address)
                <div class="text-muted mb-2">{{ $pending_order->shipping_address }}</div>
                @endif
                @if($pending_order->receiver_city)
                <div class="text-muted">{{ $pending_order->receiver_city }}{{ $pending_order->receiver_area ? ', ' . $pending_order->receiver_area : '' }}</div>
                @endif
            </div>
        </div>
        @endif
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="d-grid gap-2">
                    <button class="btn btn-success" onclick="confirmOrder({{ $pending_order->id }})">
                        <i class="fas fa-check"></i> Confirm Order
                    </button>
                    <button class="btn btn-danger" onclick="rejectOrder({{ $pending_order->id }})">
                        <i class="fas fa-times"></i> Reject Order
                    </button>
                    <a href="{{ route('admin.pending-orders.edit', $pending_order) }}" class="btn btn-warning">
                        <i class="fas fa-edit"></i> Edit Order
                    </a>
                    <button class="btn btn-outline-danger" onclick="deleteOrder({{ $pending_order->id }})">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3">
                    <strong>Order Number:</strong> <span id="confirmOrderNumber">{{ $pending_order->order_number }}</span>
                </div>
                ...
                <div class="border-top pt-3 mt-3">
                    <div class="d-flex justify-content-between mb-2">
                        <span>Subtotal:</span>
                        <strong id="confirmSubtotal">Rs. {{ number_format($pending_order->subtotal ?? $pending_order->orderItems->sum('total'), 2) }}</strong>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span>Shipping:</span>
                    ...
                    <div class="d-flex justify-content-between border-top pt-2">
                        <strong>Total:</strong>
                        <strong class="text-primary fs-5" id="confirmTotal">Rs. {{ number_format($pending_order->total, 2) }}</strong>
                    </div>
                </div>
            </div>
...
function confirmOrder(orderId) {
    ...
    const orderItems = @json($pending_order->orderItems);
    currentSubtotal = 0;
    ...
    // Set default values
    document.getElementById('confirmOrderNumber').textContent = '{{ $pending_order->order_number }}';
...
function submitConfirmOrder() {
    const orderId = {{ $pending_order->id }};
