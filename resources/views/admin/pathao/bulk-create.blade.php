...
@section('title', 'Bulk Create Pathao Shipments')
@section('page-title', 'Bulk Create Pathao Shipments')
    ...
    <div class="row">
        <div class="col-12">
            <div class="bulk-create-container">
                ...
                @if($orders->count() > 0)
                <form action="{{ route('admin.pathao.bulk-create.store') }}" method="POST" id="bulkCreateForm">
                    @csrf
                                ...
                                <tbody>
                                    ...
                                    <tr data-order-id="{{ $order->id }}">
                                        ...
                                        <td>
                                            ...
                                            <div class="order-customer">{{ $order->user->email ?? 'N/A' }}</div>
                                        </td>
                                        <td>
                                            <div class="address-cell">
                                                {{ Str::limit($order->receiver_full_address ?? $order->shipping_address ?? 'N/A', 60) }}
                                            </div>
                                        </td>
                                        <td>
                                            <select name="shipments[{{ $order->id }}][store_id]" class="form-select" required>
                                        ...
                                        <td>
                                            <select name="shipments[{{ $order->id }}][recipient_city]" 
                                        ...
                                        <td>
                                            <select name="shipments[{{ $order->id }}][recipient_zone]" 
                                        ...
                                        <td>
                                            <select name="shipments[{{ $order->id }}][recipient_area]" 
                                        ...
                                        <td>
                                            <select name="shipments[{{ $order->id }}][item_type]" class="form-select" required>
                                                <option value="1">Document</option>
                                                <option value="2" selected>Parcel</option>
                                            </select>
                                        </td>
                                        <td>
                                            <select name="shipments[{{ $order->id }}][delivery_type]" class="form-select" required>
                                        ...
                                        <td>
                                            <input type="number" 
                                                   name="shipments[{{ $order->id }}][item_weight]" 
                                                   value="0.5" 
                                                   step="0.1" 
                                                   min="0.5" 
                                                   max="10"
                                        ...
                                        <td>
                                            <input type="number" 
                                                   name="shipments[{{ $order->id }}][amount_to_collect]" 
                                                   value="{{ $order->payment_method === 'cod' || $order->payment_method === 'cash_on_delivery' ? $order->total : 0 }}" 
                                                   step="0.01" 
                                                   min="0"
                                                   class="form-control">
                                        </td>
                                        <td>
                                            <input type="text" 
                                                   name="shipments[{{ $order->id }}][recipient_phone]" 
                                                   value="{{ $order->receiver_phone ?? $order->user->phone ?? '' }}" 
                                                   class="form-control"
                                                   required>
                                            <input type="hidden" name="shipments[{{ $order->id }}][recipient_name]" value="{{ $order->receiver_name ?? $order->user->name ?? 'Customer' }}">
                                            <input type="hidden" name="shipments[{{ $order->id }}][recipient_address]" value="{{ $order->receiver_full_address ?? $order->shipping_address ?? '' }}">
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
...
document.getElementById('bulkCreateForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            alert(data.message);
            if (data.created > 0) {
                window.location.href = '{{ route("admin.pathao.index") }}';
            }
        } else {
